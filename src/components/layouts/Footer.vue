<template>
    <footer class="bg-gray-900 text-white py-10 mt-auto hidden sm:block">
        <div class="container mx-auto px-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div class="md:col-span-1">
                    <div class="flex items-center space-x-2 mb-4">
                        <div
                            class="w-8 h-8 bg-red-600 rounded-full flex items-center justify-center text-white font-bold">
                            漢
                        </div>
                        <span class="text-xl font-bold">Kanji Learning</span>
                    </div>
                    <p class="text-gray-400 text-sm mb-4">
                        Master Japanese kanji with our interactive and engaging learning platform.
                    </p>
                    <div class="flex space-x-4">
                        <a v-for="social in socialLinks" :key="social.name" :href="social.url"
                            class="text-gray-400 hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 rounded-full p-1"
                    ...
                    <ul class="space-y-2">
                        <li v-for="link in quickLinks" :key="link.name">
                            <router-link :to="link.path"
                                class="text-gray-400 hover:text-white transition-colors focus:outline-none focus:underline">
                                {{ link.name }}
                            </router-link>
                        </li>
                    </ul>
            ...
            <div class="border-t border-gray-800 mt-8 pt-6 flex flex-col md:flex-row justify-between items-center">
                <p class="text-gray-500 text-sm">
                    &copy; {{ new Date().getFullYear() }} Kanji Learning. All rights reserved.
                </p>
                <div class="flex space-x-6 mt-4 md:mt-0">
                    <a v-for="link in legalLinks" :key="link.name" :href="link.url"
                        class="text-gray-500 hover:text-white text-sm transition-colors focus:outline-none focus:underline">
                        {{ link.name }}
                    </a>
                </div>
            </div>
